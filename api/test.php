...
$base = '/app/user_data';
foreach (["$base/correct_files","$base/correct_files/excel","$base/correct_files/ppt",
          "$base/correct_files/hangul","$base/uploads","$base/results"] as $d) {
    if (!is_dir($d)) @mkdir($d, 0755, true);
}
$py3 = trim(shell_exec('which python3 2>/dev/null') ?: '');
if ($py3) {
    $ver = trim(shell_exec('python3 --version 2>&1') ?: '');
} else { $ver = 'NOT FOUND'; }
...
echo json_encode([
    ...
    'api_files'            => array_values(array_diff(scandir('/app/api'), ['.','..'])),
